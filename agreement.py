...
def V_phrase_num(tr):
    ...
    if (tr.label() == 'T' or tr.label() == 'I'):
        ...
        return tr[0][2]
    elif (((tr.label() == 'QP') and (len(tr) == 1)) or (tr.label() == 'VP')):
        return V_phrase_num(tr[0])
    elif (('WHO' == tr[0].label()) and (tr.label() == 'Rel')):
...
def check_node(tr):
    ...
    if (rule == 'S -> WHICH Nom QP QM'):
        ...
        return ('s' == N_phrase_num(tr[1]))
    elif (rule == 'NP -> Nom'):
        return ('p' == N_phrase_num(tr[0]))
    elif ((rule == 'VP -> BE NP') or ((rule == 'QP -> DO NP T'))):
